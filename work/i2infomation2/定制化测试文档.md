...
deb http://mirrors.aliyuncs.com/ubuntu/ bionic main restricted universe multiverse
deb http://mirrors.aliyuncs.com/ubuntu/ bionic-security main restricted universe multiverse
deb http://mirrors.aliyuncs.com/ubuntu/ bionic-updates main restricted universe multiverse
deb http://mirrors.aliyuncs.com/ubuntu/ bionic-proposed main restricted universe multiverse
deb http://mirrors.aliyuncs.com/ubuntu/ bionic-backports main restricted universe multiverse
deb-src http://mirrors.aliyuncs.com/ubuntu/ bionic main restricted universe multiverse
deb-src http://mirrors.aliyuncs.com/ubuntu/ bionic-security main restricted universe multiverse
deb-src http://mirrors.aliyuncs.com/ubuntu/ bionic-updates main restricted universe multiverse
deb-src http://mirrors.aliyuncs.com/ubuntu/ bionic-proposed main restricted universe multiverse
deb-src http://mirrors.aliyuncs.com/ubuntu/ bionic-backports main restricted universe multiverse
...
sudo apt-get update
sudo apt-get -y install gcc-4.8 g++-4.8  cpp-4.8  
EOF
...
sudo mv /usr/bin/g++ /usr/bin/g++-7
sudo mv /usr/bin/cpp /usr/bin/cpp-7
...
```sh
# Oracle默认不支持ubuntu需要欺骗一下Oracle安装程序，执行以下命令：
sudo mkdir -p /usr/lib64 
sudo ln -sf /etc /etc/rc.d 
sudo ln -sf /lib/x86_64-linux-gnu/libgcc_s.so.1 /lib64/ 
sudo ln -sf /usr/bin/awk /bin/awk 
sudo ln -sf /usr/bin/basename /bin/basename 
...
## 查看服务器 top10 连接客户端的 IP 地址

```sh
netstat -nat | awk '{print $5}' | awk -F ':' '{print $1}' | sort | uniq -c | sort -rn | head -n 10
```

## 查看当前服务器占用内存 最大 top10 的程序

```sh
lsof / | awk '$7 > 1048576 { print $7 / 1048576 "MB", $9, $1 }' | sort -nu | tail
```

## 查看客户端每个IP到服务器端口80的连接数
...
# 使用4个工作单元模拟CPU负载，运行5分钟
stress --cpu 4 --timeout 300

# 使用2个工作单元模拟内存负载，每个分配400MB，运行5分钟
stress --vm 2 --vm-bytes 400M --timeout 300
```

## 使用iptable模拟服务器繁忙

```sh
# 可以使用 iptables 来限制连接数，使服务器达到最大连接数后无法处理新的连接，从而模拟服务器繁忙的情况。
# 允许已建立的连接
...
# 可以使用 iptables 来限制传入或传出的数据包速率，从而模拟服务器带宽繁忙的情况。
# 限制传入数据包速率为100kbps
iptables -A INPUT -p tcp --dport 80 -m limit --limit 100kbps -j ACCEPT
...
```sh
sync 命令用于将内存中的数据同步写入到磁盘中。
echo 1 > /proc/sys/vm/drop_caches 
命令用于清理内存中的页缓存（PageCache），即清除文件系统缓存。这将导致系统需要重新从磁盘读取数据，但不会影响已分配的内存页面。

sudo sync && echo 2 > /proc/sys/vm/drop_caches：
这个命令除了清理页缓存外，还会清理目录缓存（dentries），即清除文件名的缓存。


sudo sync && echo 3 > /proc/sys/vm/drop_caches：
这个命令除了清理页缓存和目录缓存外，还会清理索引节点缓存（inodes），即清除文件属性的缓存。
```

## ubuntu下误删`/usr/include`
...
## oracle11g 32位控制文件不一致报错ORA-00214处理

```bash
[oracle@localhost ~]$ sqlplus /nolog
SQL*Plus: Release 11.2.0.4.0 Production on Wed May 22 15:33:18 2024
Copyright (c) 1982, 2013, Oracle. All rights reserved.
SQL> conn / as sysdba
Connected.
SQL> startup;
...
ORA-01507: database not mounted # 数据库一直没有挂载
ORACLE instance shut down.
SQL> startup;
ORACLE instance started.
Total System Global Area 535662592 bytes
Fixed Size 1365828 bytes
Variable Size 213911740 bytes
Database Buffers 314572800 bytes
Redo Buffers 5812224 bytes
ORA-00214: control file '/u01/app/oracle/fast_recovery_area/orcl/control02.ctl'
version 793 inconsistent with file '/u01/app/oracle/oradata/orcl/control01.ctl'
version 791   # 控制文件的版本不一致
SQL> select * from V$controlfile;
no rows selected

# 控制版本号不一致，可以看到control02.ctl的日志比control01.ctl的日志版本号小
# 先将control02.ctl文件备份，然后将control01.ctl文件复制一份为control02.ctl文件名
...
[oracle@localhost orcl]$ ls
control02.ctl
[oracle@localhost orcl]$ cp control02.ctl control02.ctl.bk
[oracle@localhost orcl]$ cp /u01/app/oracle/oradata/orcl/control01.ctl /u01/app/oracle/fast_recovery_area/orcl/control02.ctl
[oracle@localhost orcl]$ sqlplus /nolog
SQL*Plus: Release 11.2.0.4.0 Production on Wed May 22 15:43:36 2024
Copyright (c) 1982, 2013, Oracle. All rights reserved.
SQL> conn / as sysdba
Connected.
...
2. 查看具体data的文件下的最大文件夹

```sh
du -h /var/lib/docker/overlay2/ --max-depth=1 | sort -hr | head -n 10
```

## 计算一个目录的所有文件md5值

```sh
find /root/work/test/monitor2/file01-0/ -not -type d -exec md5sum {} + | awk '{print $1}' > /root/md5.txt && sort /root/md5.txt | md5sum | awk '{print $1}'
```
...
sudo dd if=/dev/zero of=swap bs=1024 count=20000000
dd 命令用于数据转换和复制。
if=/dev/zero 表示从 /dev/zero 设备读取数据，这是一个特殊文件，会输出无限的零字节。
of=swap 指定输出文件名为 swap。
bs=1024 表示每次读写的数据块大小为 1024 字节（1 KB）。
count=20000000 表示要写入的数据块数量，因此这个命令将创建一个大小为 20000000 * 1024 = 20GB 的文件。
sudo mkswap -f swap
mkswap 用于格式化一个分区或文件作为交换空间。
-f 强制格式化，即使设备看起来不像是交换空间。
swap 是要格式化的文件名。
sudo swapon swap
swapon 用于激活一个新的交换分区或文件。
swap 是要激活的交换文件名。
综上所述，这些命令创建了一个 20GB 大小的交换文件，并将其格式化为交换空间，然后启用了这个交换文件。这可以增加系统的虚拟内存容量，在物理内存不足时使用硬盘空间来扩展内存。
```

## 找出目录下所有大于50M的文件

```shell
find /root -type f -size +100M -exec du -h {} + | awk '{print $1, $2}'
```
